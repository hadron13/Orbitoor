...
int main(int argc,char*argv[]){
    ...
    window = SDL_CreateWindow("Orbitoor",SDL_WINDOWPOS_CENTERED,SDL_WINDOWPOS_CENTERED,1280,720,0);
    ...
    tragetory = SDL_CreateTexture(renderer,SDL_PIXELFORMAT_RGBA32,SDL_TEXTUREACCESS_TARGET,1280,720);
...
void menu_render(SDL_Renderer*r){
    ...
    {
    case menu:
        SDL_RenderCopy(r,menu1,NULL,NULL);
        break;
    case how:
        SDL_RenderCopy(r,how1,NULL,NULL);
        break;
    case game_over:
        SDL_RenderCopy(r,game_over1,NULL,NULL);
        break;
    case pause:
        SDL_RenderCopy(r,pause1,NULL,NULL);
        break;
    default:
        break;
    }
...
void update(void){
    ...
    for (size_t i = 0; i < asteroids_max; i++){
        ...
        if(vec_distance(zero,to_ship) - (10*scale) < 1){
            shipb.velocity = vec_add(shipb.velocity,vec_divideN(actual.body.velocity,3));
            actual.show = false;
        }
        for (size_t j = 0; j < asteroids_max; j++){
            if(j == i)
                continue;
            Asteroid actual2 = asteroids[j];
            if(!actual2.show)
                continue;
            Vector2 to_other = vec_compare(actual.body.center_pos,actual2.body.center_pos);
            if(vec_distance(zero,to_other) - (10*scale) < 1){
                actual2.body.velocity = vec_add(actual.body.velocity,vec_divideN(actual.body.velocity,2));
            }
        }
    ...
    //shipb.angle = get_angle(vec_normalized(shipb.velocity))+90;
    if(ship.right){
        shipb.angle+=3;
    ...
    if(vec_distance(shipb.center_pos,sun_pos)-(40*scale)<1){
        state = game_over;
                ...
                shipb.pos.x > 1280  ||
                shipb.pos.y < 0     ||
                shipb.pos.y > 720) state = game_over;
...
void render(SDL_Renderer* r){
    update_tragectory(2000,renderer);
    SDL_RenderCopy(renderer,tragetory,NULL,NULL);
    ...
    SDL_RenderCopyEx(r,spritesheet,(ship.engines)?&ship_fire:&ship_texture,&ship_render,shipb.angle,&center,SDL_FLIP_NONE);

    //SDL_SetRenderDrawColor(renderer,255,0,0,255);
    //SDL_RenderDrawLine(r,shipb.center_pos.x,shipb.center_pos.y,shipb.velocity.x*50+shipb.center_pos.x,shipb.velocity.y*50+shipb.center_pos.y);

    for (size_t i = 0; i < asteroids_max; i++){
        Asteroid actual = asteroids[i];
        if(actual.show){
            SDL_Rect render = {actual.body.pos.x,actual.body.pos.y,16*scale,16*scale};
            SDL_RenderCopyEx(r,spritesheet,&asteroid_textures[actual.type],&render,actual.body.angle,NULL,SDL_FLIP_NONE);
        }
    }
...
void update_tragectory(int steps,SDL_Renderer* r){
    SDL_SetRenderTarget(r,tragetory);
    SDL_SetRenderDrawColor(r,0,0,0,0);
    SDL_RenderClear(r);
    double alpha = 255.0,per_step = alpha/(double)steps;
    
    SDL_SetRenderDrawBlendMode(r,SDL_BLENDMODE_BLEND);

    Vector2 pos = shipb.center_pos,last_pos = shipb.center_pos;
    Vector2 tmp_force = shipb.velocity;
    for (size_t i = 0; i < steps; i++){
        alpha-=per_step;
        SDL_SetRenderDrawColor(r,0,255,0,(Uint8)alpha);
        tmp_force = vec_add(tmp_force,gravity(pos,shipb.mass));

        pos = vec_add(pos,tmp_force);

        SDL_RenderDrawLineF(r,pos.x,pos.y,last_pos.x,last_pos.y);
        last_pos = pos;
    }
...
void game_start(){
    ...
    ship = (Ship){ { initial_pos,{sun_pos.x,shipb.pos.y+8*scale},{1.638802,0},1,90},false,false,false};

    safe_starting = 60*5;
    //double distance = vec_distance(shipb.center_pos,sun_pos);
    //double velocity = SDL_sqrt((G*(sun_mass+shipb.mass))/distance);
    //Vector2 a = {170,170};
    //double velocity = SDL_sqrt(G*sun_mass*((2/distance)-(1/vec_distance(a,(Vector2){0,0}))));
